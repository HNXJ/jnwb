...
#### 1. RNG Convention
- Functions consuming stochasticity take an explicit parameter:
  `rng: Optional[Union[np.random.Generator, int]] = None`.
- The instance is resolved locally via:
  ```python
...
- Power: raw squared physical units ($\text{V}^2/\text{Hz}$ or $\mu\text{V}^2/\text{Hz}$); conversion to decibels is explicitly controlled via `to_db` or `aggregate_to_db` and computed last.
...
| `vflip_from_lfp` | `jnwb.laminar` | `lfp`: `(n_channels, n_times)`, `fs: float`, `band_low: Tuple`, `band_high: Tuple` | $\mu\text{V}$, $\text{Hz}$ | Strict composition: `compute_psd(lfp, fs) -> vflip(psd, freqs)`. | `VFlipResult` | Propagates PSD and vflip validation errors. | Deterministic. CPU/CUDA. | Tested for exact numerical identity against manual composition. |
