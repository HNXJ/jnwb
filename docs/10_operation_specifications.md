...
| `zflip` | `jnwb.laminar` | `lfp_matrix`: `(n_channels, n_samples)` ordered along the probe shaft, `fs: float`, `pitch_um: Optional[float] = None` | $\mu\text{m}$, $\text{Hz}$ | Computes the cross-spectra itself, then evaluates phase-gradient asymmetry and imaginary coherency across layers, reducing sensitivity to zero-phase-lag coupling. | `ZFlipResult` dataclass with `adjacent_wpli`, `adjacent_delays_s`, `adjacent_linearity_r2`, `mean_wpli`, `tau_per_channel_s`, `apparent_velocity_m_s`, `directionality`, `p_value` and `accepted`. `apparent_velocity_m_s` is an apparent phase velocity, not a conduction velocity. | Raises `ValueError` for an array that is not 2-D, for zero imaginary coherency, or for ill-conditioned cross-spectra. A `(n_channels, n_channels, n_freqs)` cross-spectrum is rejected, not accepted. | Controlled via `rng` when permutation is used. | Composes with `imaginary_coherency` and `wpli`. |
