"""
suite_01_raster_s_om.py — Raster suite of S+/S-/O+ neurons
Runs in a loop over all sessions matching visual / frontal areas, resolves classifications
using the template scan and grand unit table, and saves figures per session prefix.
Usage:
  python scripts/suite_01_raster_s_om.py
"""
...
def main():
    ...
    for _, row in active_sessions.iterrows():
        ...
        # Filter classifications for this session
        sub = grand[grand["nwb_stem"] == stem]
        if len(sub) == 0:
            continue
            
        # Select representative units for S+, S-, O+ dynamically based on classification
        ...
        if len(s_plus_candidates) == 0 or len(s_minus_candidates) == 0 or len(o_plus_candidates) == 0:
            # Fall back to top templates or anything available
        ...
        for row_fig, cond in enumerate(CONDITIONS):
            ...
            for col_idx, (cls, unit_spec) in enumerate(units.items()):
                ...
                if omission_epoch is not None:
                    ...
                    # Plot 'x' marker just below the bottom of the raster (outside clip region, y = N_TRIALS_SHOWN + 1.5)
                    ax_raster.plot(center_om, N_TRIALS_SHOWN + 1.5, marker="x", color="red", markersize=6, markeredgewidth=1.5, clip_on=False, zorder=10)
                ...
                for onset in shuffled_onsets:
                    ...
                    base_spk = np.sum((rel_ms >= -500.0) & (rel_ms < 0.0))
                    stim_spk = np.sum((rel_ms >= 0.0) & (rel_ms < 531.0))
                    base_rates.append(base_spk / 0.5)
        ...
        fig.suptitle(f"Suite 01: S+ / S- / O+ Raster & PSTH Grid — {prefix}", fontsize=14, fontweight="bold", y=0.96)
