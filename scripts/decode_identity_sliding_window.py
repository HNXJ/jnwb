...
Three analyses, all sessions x all areas x all units, reusing jnwb.structured_identity's generic
per-session trial ontology (NOT jnwb.structured_identity_m2a's frozen 5-session Milestone 2A
design, which stays untouched and out of scope here):
  ...
  3. reversal_generalization -- train on p2+p3 (where preceding==expected), test on p4 (where
     ...
     `cross_position_cycle` rule (jnwb.omission_identity.detect_trial_cycles on each session's own
     combined p2/p3/p4 eligible timestamps).

CV grouping uses each session's own `cycle` field (jnwb.omission_identity.detect_trial_cycles via
...
from jnwb.omission_identity import detect_trial_cycles
from jnwb.structured_identity import build_canonical_trial_table, MAIN_ANALYSIS, POSITIVE_CONTROL
...
def build_instance_tables(sess) -> dict[str, pd.DataFrame]:
    ...
    (jnwb.omission_identity.detect_trial_cycles on each session's own combined p2/p3/p4 eligible
