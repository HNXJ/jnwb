"""
scripts/build_figure3_raster_grid_exact_pulse_v2.py

Generates a version of the Figure 3 Raster Grid using exact-pulse matched units with a low-rate S- unit:
- S+ Unit: 161 (unit_id column 5 in MT, ~11.7 Hz overall rate, r_S+ = 0.93)
- S- Unit: 362 (unit_id column 193 in V2, ~5.0 Hz overall rate, r_S- = 0.96)
...
def main():
    ...
    for row_i, cond in enumerate(CONDITIONS):
        for col_i, cls in enumerate(["S+", "S-", "O+"]):
            ...
            if row_i == 0:
                ax_raster.set_title(f"{cls} Candidate ({areas[cls]})", fontsize=11, fontweight="bold", color=CLASS_COLORS[cls], pad=15)
                # Dual time axis top
    ...
    fig.suptitle(f"Figure 2: Exact-Pulse Matched Raster Grid (O+/S+/S- @ ~5-13.6 Hz)\nCausally Exponential-Smoothed Average Firing Rate (AFR) Profiles", fontsize=13, fontweight="bold", y=0.97)

    svg_path = OUT_DIR / "figure2_raster_grid_exact_pulse_low_rate_Sminus.svg"
    png_path = OUT_DIR / "figure2_raster_grid_exact_pulse_low_rate_Sminus.png"
