...
def main():
    ...
    parser.add_argument("--tiny-npy-smoke", action="store_true", help="Run a self-contained tiny .npy file reader smoke test using a temporary file")
    parser.add_argument("--out", help="Optional report file path to save output JSON")
    args = parser.parse_args()

    print("Initializing Phase 2J Bounded Signal Slice CLI validator...")
    ...
    temp_file_path = None
    if args.tiny_npy_smoke:
        fd, temp_file_path = tempfile.mkstemp(suffix=".npy")
        os.close(fd)
        dummy_arr = np.ones((5, 6, 150), dtype=np.float32)
        np.save(temp_file_path, dummy_arr)
        args.allow_real_data = True
        args.source_path = temp_file_path
