"""
scripts/build_figure3_raster_grid_better_matches.py

Generates a version of the Figure 3 Raster Grid using firing-rate-matched units:
- S+ Unit: 103 (~8.1 Hz overall mean rate)
- S- Unit: 275 (~7.0 Hz overall mean rate)
- O+ Unit: 52 (~9.7 Hz overall mean rate, unit_id column 53 in FEF)
...
def plot_raster_subplot(ax_raster, ax_psth, spike_times: np.ndarray, onsets: np.ndarray, color: str, condition: str):
    ...
    ax_psth.set_ylim(0, 45)  # capped at 45 Hz to show similar scales
...
def main():
    ...
    # Firing rate matched S+, S-, O+ units (better matches near 10 Hz)
    units = {"S+": 103, "S-": 275, "O+": 52}
    ...
    fig.suptitle(f"Figure 2: Firing-Rate-Matched Raster Grid (O+/S+/S- @ ~10 Hz)\nCausally Exponential-Smoothed Average Firing Rate (AFR) Profiles", fontsize=13, fontweight="bold", y=0.97)

    svg_path = OUT_DIR / "figure2_raster_grid_mean_matched_better_O.svg"
    png_path = OUT_DIR / "figure2_raster_grid_mean_matched_better_O.png"
