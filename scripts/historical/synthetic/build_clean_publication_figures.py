...
Per artifacts/.lab/agent-harness-audit-20260810.json (Sol/Hamm Handout 2, P0 item 3) and
context/docs/CONTEXT.md line ~221 ("Do not use scripts/build_clean_publication_figures.py --
it contains synthetic placeholder construction"): already named-and-banned in project doctrine;
this move makes that ban structurally enforced (quarantine dir + machine-readable marker) rather
than relying on an agent having read CONTEXT.md. Preserved per Conservation doctrine, not
deleted.

scientific_status = "invalid_for_inference"
superseded_by = None
reason = ["synthetic_placeholder_construction"]
