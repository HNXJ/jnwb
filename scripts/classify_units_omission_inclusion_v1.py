...
(jnwb.unit_classification.classify_unit, unmodified) for S+/S-/local-baseline O+/O++; (2) the
new likelihood-of-firing criterion (jnwb.unit_inclusion.classify_unit_omission_inclusion) --
...
from jnwb.unit_classification import OM_BASE_GAP_MS
...
def run(max_sessions: int = None) -> pd.DataFrame:
    ...
    for si, row in enumerate(ready.itertuples(index=False), start=1):
        ...
        from jnwb.unit_classification import _assign_labels
    ...
    manifest = {
        ...
        "old_criterion_scope": (
            ...
            "fx=0 in its O+ template -- not jnwb.unit_classification's own local-baseline O+, "
