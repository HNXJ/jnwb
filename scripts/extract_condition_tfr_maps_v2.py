...
OUT_DIR = _P.REPO_ROOT / "outputs/condition_tfr_maps_p1d1p2d2p3_v2"
...
def main(limit=None):
    ...
    for k, (f, g, chans, lmap) in enumerate(targets, 1):
        ...
        for c0 in range(0, sel.size, CHUNK):
            ...
            ch_block_pos = sel_pos[c0:c0 + CHUNK]
            buf = np.asarray(arr[:, ch_block_pos, :, i0:i1], dtype=np.float32)  # (trials,nch,f,t)
    ...
    receipt = {
        ...
        "purpose": "RXRR/RRXR-vs-RRRR dB maps per session x area x putative layer, p1-aligned, "
                   ...
                   "file's own 'channels' array, not by array position) -- supersedes "
                   "outputs/condition_tfr_maps_p1d1p2d2p3/maps.npz, whose receipt shows it was "
                   "generated 2026-08-04 from the superseded D:/workspace/data/tfr_arrays path.",
