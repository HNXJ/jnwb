"""
scripts/build_figure3_raster_grid_mean_matched.py

Generates a version of the Figure 3 Raster Grid using firing-rate-matched units:
- S+ Unit: 25 (~21.9 Hz overall mean rate)
- S- Unit: 318 (~18.4 Hz overall mean rate)
- O+ Unit: 41 (~20.3 Hz overall mean rate)
...
def causal_exponential_smoothing(spike_times_rel: list[float], n_trials: int, window_ms: tuple[float, float], tau_ms: float = 30.0, bin_ms: float = 1.0) -> tuple[np.ndarray, np.ndarray]:
    ...
    # Shift/pad so that it is causal (only convolving backward in time)
    # scipy convolve1d with origin shifted
...
def plot_raster_subplot(ax_raster, ax_psth, spike_times: np.ndarray, onsets: np.ndarray, color: str, condition: str):
    ...
    ax_psth.set_ylim(0, 50)  # capped at 50 Hz to show similar scales
...
def main():
    ...
    # Firing rate matched S+, S-, O+ units
    units = {"S+": 25, "S-": 318, "O+": 41}
    ...
    # Each cell is split into two rows: top raster (3x height) and bottom PSTH (1x height)
    ...
    for row_i, cond in enumerate(CONDITIONS):
        for col_i, cls in enumerate(["S+", "S-", "O+"]):
            # Split the outer cell cell into raster + PSTH
    ...
    fig.suptitle(f"Figure 2: Firing-Rate-Matched Raster Grid (O+/S+/S- @ ~20 Hz)\nCausally Exponential-Smoothed Average Firing Rate (AFR) Profiles", fontsize=13, fontweight="bold", y=0.97)

    svg_path = OUT_DIR / "figure2_raster_grid_mean_matched.svg"
    png_path = OUT_DIR / "figure2_raster_grid_mean_matched.png"
