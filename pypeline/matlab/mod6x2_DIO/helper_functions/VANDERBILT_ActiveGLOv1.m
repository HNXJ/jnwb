...
for ii = 1 : numel(trial_start)
    ...
    active_flag = find(contains(task_data.event_code_type(trial_start(ii):trial_end(ii)), 'task_event_7'));
    cntActive = cntActive + active_flag;
...
%         contains(task_data.event_code_type(trial_start(ii):trial_end(ii)), '5')));
    ...
    disp("ID->" + num2str(ii));
%     disp(task_data.event_code_type(trial_start(ii):trial_end(ii)));
%     disp(cntActive);
%     disp(presentations);
%     disp(numel(presentations));

    if numel(presentations) == 6
        complete_trial = 1;
        prez_seq = task_data.orientation(presentations(1:4)+(trial_start(ii)-1))';
%         disp(prez_seq);
        task_data.seq_type(trial_start(ii):trial_end(ii)) = find(sum(seq_combos == prez_seq,2) == 4);
%         disp(find(sum(seq_combos == prez_seq,2) == 4));
        lCnt = lCnt + 1;
        disp(lCnt);
    elseif (numel(presentations) == 5) && (isempty(active_flag))
        complete_trial = 1;
        prez_seq = task_data.orientation(presentations(1:4)+(trial_start(ii)-1))';
        task_data.seq_type(trial_start(ii):trial_end(ii)) = find(sum(seq_combos == prez_seq,2) == 4);
    else
