function event_data = identEvents(codes, times, task_ident_code, task_trial_start_code, task_trial_end_code)
    ...
    try
        ...
        disp("-->Skipping strobe position shift (no shifts detected)");
        
    end
    
%     try % Repair section : Used for C31-2023-oGLOPassive-0830/0831
%     
%     %     bhvPath = "bhvData\";
%     %     bhvFiles = {dir(bhvPath).name};
%     %     bhvFiles = bhvFiles(endsWith(bhvFiles, ".nwb"));
%     %     bhvFile = bhvPath + bhvFiles{1};
%     
%         bhvFile = "230831_Cajal_glo_omission.bhv2";
%         
%         offset_time_adj = 1565.111; % % Offsets> 230830:1378.856 | 230831:1565.111
%         bhvs = mlconcatenate(bhvFile);
%         nt = size(bhvs.BehavioralCodes, 1);
%         cnt = length(assigned_task);
%     
%         for kk = 1:nt
%     
%             trialified{kk + cnt}.codes = bhvs.BehavioralCodes(kk).CodeNumbers;
%             trialified{kk + cnt}.times = (bhvs.BehavioralCodes(kk).CodeTimes / 1000) + offset_time_adj;
%             assigned_task(kk + cnt) = 9;
%     
%         end
%     
%         disp("-->Behavioral codes repaired.");
%     
%     catch
%     
%         disp("->BHV");
...
%     end
    ...
    for ii = 1:numel(tasks)
        ...
        try
            ...
            disp("-->Processing the events for task code : " + num2str(tasks(ii)));
            
            if numel(event_data{ii}.start_time) ~= numel(event_data{ii}.stop_time)
               
                warning("--->Event stop/start times must have same number of elements.");
                warning("--->Start event count = " + num2str(numel(event_data{ii}.start_time)));
                warning("--->Stop event count = " + num2str(numel(event_data{ii}.stop_time)));
                warning("--->Replaced stop times with shifted start times to proceed.");
                
                event_data{ii}.start_time = event_data{ii}.start_time(2:end);
                event_data{ii}.start_time(end + 1) = event_data{ii}.start_time(end) + 1;
                event_data{ii}.stop_time = event_data{ii}.start_time(2:end);
                event_data{ii}.stop_time(end + 1) = event_data{ii}.stop_time(end) + 1;
                
                disp("-->Event codes processed for task code : " + num2str(tasks(ii)) + " start/stop times might require to be fixed.");

            else

                disp("-->Event codes processed for task code : " + num2str(tasks(ii)));
