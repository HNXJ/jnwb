    ...
    for jj = 1 : size(secondary_codes, 1)
        ...
        if secondary_codes(jj,1) > 1000
            ...
            if isempty(sci)
                ...
                warning('--->Secondary code skipped.');
            end
        else
        ...
        if strcmp(scs_fields{jj}, 'task')
            ...
            if isempty(tci)
                if exist('task_input', 'var')
                    try
                        scs.(scs_fields{jj}) = ['GENERAL_EVENTS_', task_input];
                    catch
                        scs.(scs_fields{jj}) = task_input;
                    end
                else
                    scs.(scs_fields{jj}) = 'GENERAL_EVENTS';
                end
            else
        ...
        if (primary_codes(jj) < 100 | primary_codes(jj) > 250) & primary_codes(jj) ~= 10
            ...
            if primary_codes(jj) == 18
                ...
                try
                    ...
                    disp("->Skipping secondary event codes (task not listed previously)");
            ...
            if stimulus_specific_codes{kk}(mm,1) > 2000 % bug in output code
                ...
                try
                    ...
                    elseif kk > 1 | mm < numel(stimulus_specific_codes)
                        ...
                        warning('cannot parse stimulus specific event code; Ignore this warning if you do not need secondary annotations')
                        continue
                    end
                catch
                    warning('--->Secondary code skipped.')
