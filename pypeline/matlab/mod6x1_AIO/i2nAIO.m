...
%             warning('->Reward detection might be inaccurate');
            threshold_val = 1;
    ...
    if contains(lower(adc_map(jj)), 'laser')
        laser_ctr = laser_ctr + 1;

        laser_state = types.core.TimeSeries( ...
            'description', 'Laser electrical state in V', ...
            'data', temp_dat, ...
            'data_unit', 'Volts', ...
            'starting_time_rate', 1000, ... % Hz
            'timestamps', recdev.time_stamps_s_ds, ...
...
%             laser_type_id{ls,1} = [num2str(frequency_ct(ls)) '_hz_wave'];
%         end
