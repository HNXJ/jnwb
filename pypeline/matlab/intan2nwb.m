...
function intan2nwb(ID,IMAGE_TOKEN,varargin)
    ...
    for ii = to_proc
        ...
        disp(['Processing : ' recording_info.Identifier{ii}]);
        ...
        if strcmp(recording_info.Raw_Data_Format{ii}, 'AI-NWB')
            ...
            if ~exist(raw_data_dir, 'file')
                warning('Raw data (Allen Ins.) not detected.')
                continue
            end
        ...
        try
            ...
            if send_slack_alerts
                SendSlackNotification( ...
                        SLACK_ID, ...
                        [nwb.identifier ': [' s2HMS(ttt) '] Error in recording sheet entry or folder name; processing failed on this session.'], ...
                        'preprocess', ...
                        'iJakebot', ...
                        '', ...
                        ':robot_face:');
                ...
                try
                    ...
                    try
                        ...
                        if send_slack_alerts
                            SendSlackNotification( ...
                                SLACK_ID, ...
                                [nwb.identifier ': [' s2HMS(ttt) '] Missing data : LFP/MUA filtering failed on probe no. ' num2str(rd-1)], ...
                                'preprocess', ...
                                'iJakebot', ...
                                '', ...
                                ':robot_face:');
        ...
        try
            ...
            disp("Validation failed, ignore if the task is not included in the validation.");
        end
