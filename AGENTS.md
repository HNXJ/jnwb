...
whether the one on disk was built at the current HEAD. The script exists because the same basis
was reconstructed twice in one day into agent transcripts, where the next packet could not read
it.
...
- **Prepare** — load, in order: (1) `AGENTS.md` (this file), (2) `artifacts/goal.md`,
  (3) `artifacts/fact_stack.md`, (4) `artifacts/state.md`, regenerating it first if
  `scripts/reconstruct_state.py --check` fails, (5) `artifacts/problem_stack.md`,
  (6) `artifacts/todo_stack.md`, (7) relevant skills (§7), (8) current repository evidence
