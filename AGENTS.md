...
| `scripts/harness_gate.py` | Repository gates 1–14 (§6) |
...
| problem | `artifacts/problem_stack.md` | Defects, contradictions and unknowns as found | Yes — add on discovery; close only by repairing, accepting with a reason, or showing the premise false |
...
P-C7 in `artifacts/problem_stack.md`.
...
| `python scripts/harness_gate.py` | Gates 1–14, in order | Boundary, skills, paths, root, docs, API set, versions, Python policy, import shadowing, forbidden study tokens in Gate 6 scan surface, NWB onboarding alignment, internal process vocabulary in `docs/` |
