...
- `jnwb.phase_locking_index(unit_spike_times, lfp_phase, lfp_timestamps, n_bins=18)`: Spike-field phase locking index. With no spikes it reports `pli` 0.0 and `preferred_phase` 0.0 rather than NaN, with `n_spikes` 0: check `n_spikes` before reading a phase.

- `jnwb.bin_spikes(spike_times, window_s=None, bin_size_ms=10.0, trial_starts=None, output="count", return_centers=False)`, `jnwb.fires_in_window(spike_times, onset_s, window_ms)`, `jnwb.rate_in_window(spike_times, onset_s, window_ms)` and `jnwb.fire_indicator(spike_times, onsets_s, window_ms)`: The half-open binning family. Every window is `[start, end)`, so a spike on a boundary belongs to exactly one bin and adjacent windows cannot both count it -- the double count `docs/common_mistakes.md` section 2 exists for. Use these rather than open-ended comparisons. `bin_spikes` requires `window_s` although the signature allows `None`: omitted, it raises `ValueError`.
