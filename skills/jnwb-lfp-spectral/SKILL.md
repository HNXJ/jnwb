...
- `jnwb.relative_power(power, baseline, *, model="mean_of_ratios", axis=None, device="cpu")`: Power against baseline under an explicit estimand. `"mean_of_ratios"` and `"ratio_of_means"` are different quantities, not two routes to one; the model is named in the result.
...
- `jnwb.laplacian_reference(channel_data, channel_order=None)`: 1-D nearest-neighbour Laplacian re-reference along the probe's depth order. Pass `channel_order` unless the array is already in depth order, or the reference is spatially meaningless.
