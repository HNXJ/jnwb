...
- `jnwb.TFRAccumulator.add_trial(self, z, valid=None, *, baseline=None)` then `jnwb.TFRAccumulator.mean_of_ratios(self)`: The streaming `how="mean_of_ratios"`. Pass each trial's own baseline power with the trial; `to_db(acc.mean_of_ratios())` equals `aggregate_to_db(how="mean_of_ratios")` over the stacked trials. `acc.power()` cannot give it: `aggregate_to_db` refuses trial-averaged power for this estimand.
