...
- `jnwb.granger_causality(signal1, signal2, order=5, device="cpu", ridge=0.0, criterion="aic")`: Raw bivariate Granger values, with no surrogate testing. Prefer `granger`, which carries it. A fixed `order` is an integer >= 1 in all three Granger calls; `0`, a fraction or a bool raises. Directional language is licensed by prediction improvement, not by causation.
