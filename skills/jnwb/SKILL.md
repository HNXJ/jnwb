...
- **GPU**: Operations whose signature takes `device` accept `device='cuda'`. With a GPU present, `complex_tfr`, `relative_power`, `spectral_tilt`, `wpli`, `imaginary_coherency`, `harmonic_analysis` and `granger_causality` compute on it; only `complex_tfr` records `device='cuda'` on its result, so a missing device field does not mean the CPU. A request no GPU can serve warns and runs on the CPU. `rdm` warns and returns a plain array. `jrsa` computes every metric in NumPy on the CPU whatever its `backend` or `device`; a `device='cuda'` request warns and records `execution['device'] == 'cpu'`.
