...
- **GPU**: Operations whose signature takes `device` accept `device='cuda'`. With a GPU present, `complex_tfr`, `band_power`, `relative_power`, `spectral_tilt`, `wpli`, `imaginary_coherency`, `cross_area_coherence`, `harmonic_analysis`, `granger_causality`, `UnitAnalyzer.autocorrelogram` and `PopulationAnalyzer.population_trajectory` compute on it; only `complex_tfr` (`device`) and `cross_area_coherence` (`device_used`) record the device on their result, so a missing device field does not mean the CPU. A request no GPU can serve warns and runs on the CPU. `rdm` and `vflip` warn and run on the CPU even with a GPU present. `jrsa` computes every metric in NumPy on the CPU whatever its `backend` or `device`; a `device='cuda'` request warns and records `execution['device'] == 'cpu'`.
