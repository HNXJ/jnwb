...
- **`compress_fp32` requires `select=` (breaking).** `select=` is keyword-only with no default;
  a call that omits it, or passes `select=None`, raises `TypeError` before anything is written.
  The anchored LFP/MUAE preset and its `FutureWarning` are removed. Callers name the
  floating-point datasets to cast, or pass `select=[]` to cast nothing, which is also the route
  for a file whose LFP is stored as integers.
