...
| 4 | `fig04_omission_identity_decoding/` | yes | no | **P0 leakage-safe rerun in progress** | **PROMOTED 2026-08-06** from supplement `figS24_omission_identity_decoding`. The old random-CV `_v2` result is confounded (~0.601); the cycle-deconfounded result is approximately chance (~0.495). The renderer now refuses missing/incomplete leakage-safe artifacts and has no scientific fallback values. Do not cite until the complete-corpus receipt, permutation null, and visual review exist. |
...
above), and the new fig04 remains unpublishable until its complete leakage-safe receipt and
visual review exist.
