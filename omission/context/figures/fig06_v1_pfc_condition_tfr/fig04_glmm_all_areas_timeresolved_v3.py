...
cell-for-cell against both v1 (stale corpus) and v2 (correct corpus, no artifact repair).
...
WHY
    fig04_glmm_all_areas_timeresolved.py (this file's non-v2 sibling) reads
    outputs/condition_tfr_maps_p1d1p2d2p3/maps.npz, whose own receipt.json shows it was
    generated 2026-08-04 from source_dir="D:/workspace/data/tfr_arrays" -- a path
    context/PROJECT_STATE.md explicitly lists under "Superseded paths -- do not restore". That
    run (lab node fig04-glmm-all-areas-timeresolved-20260813.json, status "confirmed") therefore
    predates both the 2026-08-08 TFR path migration and the 2026-08-11 precompute_tfr_arrays_v2.py
    rebuild (.npy->.npz, 4th probe letter, 1/f-quality channel screening). Per labyrinth doctrine
    ("re-verify independently rather than inheriting a prior seal's verdict"), that confirmed
    status is not trustworthy against the current corpus and this script re-derives it rather
    than assuming it replicates.
    Regenerated input: scripts/extract_condition_tfr_maps_v2.py ->
    outputs/condition_tfr_maps_p1d1p2d2p3_v2/maps.npz (485/485 files processed, 0 skipped,
    900 keys, all 11 AREA_ORDER areas present, generated 2026-08-14 from the current
    D:\analysis\tfr_arrays corpus).
    Only the CONDITION_MAPS patch and OUT_DIR differ from the original below; model code,
    windowing, and family declarations are unchanged so the two runs are comparable cell-for-cell.
...
_fig04.CONDITION_MAPS = os.path.join(REPO, "outputs", "condition_tfr_maps_p1d1p2d2p3_v3", "maps.npz")

P2_WINDOW_MS = (1031.0, 1562.0)
WINDOW_BIN_MS = 150.0
OUT_DIR = os.path.join(REPO, "outputs", "fig04_glmm_all_areas_timeresolved_v3")
