# Figure 6 — V1/V3a-d/TEO/PFC time-frequency, RXRR vs RRRR

**Renumbered 2026-08-06: this was fig04, now fig06.** `figS24_omission_identity_decoding`
was promoted into the vacated fig04 slot (`../fig04_omission_identity_decoding/`); this
figure moved to fig06 to make room, and the old fig06 (SPK-SPK lead/lag coupling) was demoted
to a supplement (`../spk_spk_coupling_supplement/`). Internal script/output filenames below
still say `fig04` (e.g. `fig04_v1_pfc_condition_tfr.py`, `fig04.svg`, `svg/fig04_*_stats.md`)
and are NOT renamed — same convention as the earlier `band_power_hierarchy_supplement/`
renumbering: the directory name is the source of truth for figure identity, internal
...
context groups, each pooling trials across the three block-type families (R, A, B) at the
shared p2 slot: **stim** = RRRR+AAAB+BBBA (no omission anywhere), **omission** =
RXRR+AXAB+BXBA (p2 omitted in all three). Both groups already share the same p1-relative time
base (p2 sits at 1031-1562 ms in every condition of both groups), so no slot-realignment is
needed — a narrower, confirmed-scope case of the broader R/A/B/slot-position realignment in
`../PLAN_sliding_window_connectivity.md`, which remains a separate, larger, not-yet-done
follow-up. Reuses `condition_tfr_maps_p1d1p2d2p3/maps.npz` directly (already covers all 7
needed conditions for all 10 areas) — no new NWB extraction.

Visually confirms two things already found by other methods this week: a real-stimulus gamma
burst (~40-80 Hz) at every real slot, cleanly ABSENT at the omitted p2 slot in the omission
context (V1/V2) — physiological sanity check that the family pooling is correctly aligned —
and, in FEF/PFC, a gamma elevation that becomes more visible post-omission alongside an
alpha/beta suppression specific to the omission context, consistent with (not a re-derivation
of) fig05's subject-controlled frontal low-gamma finding via a completely different pooling
scheme. Output: `svg/fig04xx_pair_*.svg/.png`, `svg/fig04xx_receipt.json`.
