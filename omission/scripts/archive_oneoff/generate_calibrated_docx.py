"""
Applies derived exact Clopper-Pearson 95% CIs and true session-level counts to a new document
omission-2026-draft-calibrated.docx (to avoid Windows file lock on the open docx file).
"""
...
OUT_PATH = REPO / 'context' / 'omission-2026-draft-calibrated.docx'
...
for p in doc.paragraphs:
    if p.text.startswith('Omission paradigms provide a unique window'):
        ...
        p.text = (
            ...
            f"was a selective minority ({o_cnt}/8,597 units, {o_pct}%, 95% Clopper-Pearson CI [{o_ci[0]}%, {o_ci[1]}%]), "
            "concentrated primarily in higher-order prefrontal (PFC: 104 units) and frontal eye field (FEF: 98 units) circuits. In contrast, "
            f"local field potentials exhibited sustained, hierarchy-wide low-frequency power perturbations (beta 14–30 Hz: {beta_cnt}/8,736 channels, "
            ...
            f"({gamma_pct}%, 95% CI [{gamma_ci[0]}%, {gamma_ci[1]}%]). Exploratory spectral Granger causality "
...
for p in doc.paragraphs:
    if p.text.startswith('Across the 21-session dataset'):
        ...
        sso_sp_cnt = s_6655['S+']['count']
        sso_sp_pct = s_6655['S+']['percentage']
        sso_sp_ci = s_6655['S+']['ci_95']

        sso_sm_cnt = s_6655['S-']['count']
        sso_sm_pct = s_6655['S-']['percentage']
        sso_sm_ci = s_6655['S-']['ci_95']

        sso_op_cnt = s_6655['O+']['count']
        sso_op_pct = s_6655['O+']['percentage']
        sso_op_ci = s_6655['O+']['ci_95']
        ...
        p.text = (
            ...
            f"shuffle test yielded {sso_sp_cnt} S+ ({sso_sp_pct}%, 95% CI [{sso_sp_ci[0]}%, {sso_sp_ci[1]}%]), "
            f"{sso_sm_cnt} S- ({sso_sm_pct}%, 95% CI [{sso_sm_ci[0]}%, {sso_sm_ci[1]}%]), and "
            f"{sso_op_cnt} O+ ({sso_op_pct}%, 95% CI [{sso_op_ci[0]}%, {sso_op_ci[1]}%]) units."
        )
...
print("Successfully generated calibrated manuscript:", OUT_PATH)
