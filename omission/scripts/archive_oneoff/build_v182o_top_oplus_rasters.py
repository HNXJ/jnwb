"""
scripts/build_v182o_top_oplus_rasters.py

Generates raster grids with causal-smoothed PSTHs for the five requested V182o top O+ units.
"""
...
def main():
    ...
    targets = [
        {"prefix": "sub-V182o_ses-260708", "uid": 258, "ks_id": 46.0, "area": "V4", "rate": 3.23, "qual": "MUA"},
        {"prefix": "sub-V182o_ses-260629", "uid": 212, "ks_id": 21.0, "area": "TEO", "rate": 2.41, "qual": "Good"},
        {"prefix": "sub-V182o_ses-260706", "uid": 176, "ks_id": 53.0, "area": "V4", "rate": 2.73, "qual": "MUA"},
        {"prefix": "sub-V182o_ses-260708", "uid": 324, "ks_id": 20.0, "area": "TEO", "rate": 1.07, "qual": "MUA"},
        {"prefix": "sub-V182o_ses-260708", "uid": 255, "ks_id": 43.0, "area": "V4", "rate": 4.00, "qual": "Good"}
    ]

    for unit_info in targets:
        prefix = unit_info["prefix"]
        uid = unit_info["uid"]
        ks_id = unit_info["ks_id"]
        area = unit_info["area"]
        rate = unit_info["rate"]
        qual = unit_info["qual"]
        ...
        # Note: session.get_spike_times uses the DataFrame index directly, which is unit_idx!
        # In our targets dict, the 'uid' holds the DataFrame row index (unit_idx).
        ...
        for row_i, cond in enumerate(CONDITIONS):
            ...
            if row_i == 0:
                ax_raster.set_title(f"O+ Candidate ({area}, KS {ks_id}) in {prefix}\nRate: {rate} Hz ({qual})", fontsize=11, fontweight="bold", pad=15)
                # Dual time axis top
        ...
        svg_path = OUT_DIR / f"figure2_{prefix}_unit{ks_id}_oplus_raster.svg"
        png_path = OUT_DIR / f"figure2_{prefix}_unit{ks_id}_oplus_raster.png"
