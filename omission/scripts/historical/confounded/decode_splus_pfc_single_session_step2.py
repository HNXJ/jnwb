...
r"""
Step 2: same session, same 19 S+ PFC units as step 1, but now:
  - WIDE windows spanning delay-slot-delay, not just the ~530ms presentation window:
      p2 slot -> d1-p2-d2 = [531, 2062) ms   (AXAB / BXBA / RXRR)
      p3 slot -> d2-p3-d3 = [1562, 3093) ms  (AAXB / BBXA / RRXR)
      p4 slot -> d3-p4-d4 = [2593, 4124) ms  (AAAX / BBBX / RRRX)
    (boundaries from figstyle.EPOCH_ONSETS_MS; each window's end is the next slot's onset --
    d2 legitimately appears in both the p2 and p3 windows, as specified.)
  - ONE-SHOT classification: one SVM per slot, one feature vector per trial (spike count over
    the whole wide window, not a sliding time window), 3-way (A vs B vs R), not binary.
  - Same half/half (2-fold) CV convention as step 1.
"""
...
def build_matrix(session, unit_ids, epochs_by_class, win_ms):
    onsets, labels = [], []
    for lab, (cls, epochs) in enumerate(epochs_by_class.items()):
        onsets.append(epochs["start_time"].values)
        labels.extend([lab] * len(epochs))
...
def main():
    ...
    for slot_key, cfg in SLOTS.items():
        ...
        n_by_class = {c: len(e) for c, e in epochs_by_class.items()}
        print(f"=== slot {slot_key}  (window {win[0]:.0f}-{win[1]:.0f} ms)  "
             f"conditions A={cfg['A']} B={cfg['B']} R={cfg['R']} ===")
        print(f"  n trials: {n_by_class}")

        X, y = build_matrix(session, unit_ids, epochs_by_class, win)
        ...
        chance = max(n_by_class.values()) / sum(n_by_class.values())
        print(f"  mean 2-fold accuracy: {np.mean(fold_accs):.3f}  "
             f"(chance/majority-class baseline = {chance:.3f})")
