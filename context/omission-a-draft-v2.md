...
With three subjects, subject cannot support a random-effect variance component — three levels
cannot identify one. Subject is therefore handled by stratification, by fitting each animal
separately, and by within-subject contrasts, and never by a three-level random term.
...
### Low-frequency power is strongly modulated at every recording site, with no direction shared across the hierarchy
...
1.18 dB in alpha, 1.16 dB in theta and 0.91 dB in beta, against 0.59 dB in low gamma and 0.42 dB
in high gamma (293,760 channel × band × condition measurements from 711 condition files, 17
sessions, median 39 trials per channel). Modulation of this size was present in every area
sampled. The low-frequency bands are modulated roughly twice as strongly as the gamma bands, which
is the pattern the predictive-routing account anticipates.

The direction of that modulation, however, is not shared. Averaging the signed change across areas
returns approximately zero in every band — theta −0.53 dB (P=0.07), alpha −0.37 dB (P=0.18), beta
−0.22 dB (P=0.28), low gamma −0.06 dB (P=0.55), high gamma +0.13 dB (P=0.07), n = 17 sessions —
because areas move in opposite directions and cancel. We emphasise that this pooled estimate tests
whether the hierarchy shares a common sign, and not whether individual areas are modulated; the two
questions have opposite answers here, and conflating them would misdescribe the data in either
direction.

The same split appears between animals. Fitted separately within each animal at the session level,
low-frequency power fell in one — theta −1.58 dB, alpha −1.36 dB, beta −0.89 dB, low gamma
−0.39 dB, all q < 0.005 across 8 sessions — and rose in another, with theta +0.56 dB (P=0.049) and
low and high gamma each near +0.27 dB (q < 0.002) across 4 sessions, while the third showed no
band reaching significance across 5 sessions. Because no cortical area in this corpus was recorded
in all three animals, area and animal cannot be separated, and we do not attempt to attribute the
divergence to either.

One methodological point constrains the magnitudes above. The low-frequency response does not
resolve within the omitted slot; it continues to develop through the following delay, so a measure
confined to 0–531 ms samples its leading edge. [[STAT: band × area estimates in the omitted-slot
window, the post-omission delay window, and their union, from the time-resolved maps; the
post-omission values are expected to exceed the omitted-slot values reported above]]

[[FIG 6 — omission-aligned time-frequency structure across all ten areas; see caption]]

### Elevated alpha and beta in V3 is the largest area-level departure but rests on one animal

The one area-level departure large enough to matter is in V3. Relative to V1, and estimated at the
session level, V3 power during the omitted slot was higher in beta by +1.82 dB (q=1.6 × 10⁻⁶), in
alpha by +1.32 dB (q=0.006) and in low gamma by +0.52 dB (q=0.004). No other area reached
comparable magnitude; the next largest were PFC in alpha (+1.02 dB, q=0.03) and FEF in low gamma
(+0.44 dB, q=0.02).

These estimates cannot be read as area effects on their own, for the reason given above: the
recording design confounds area with animal. The V3 effect can be tested with animal held constant
in exactly one place. In one subject, a V3 probe and a V1/V2 probe were recorded simultaneously in
the same five sessions, allowing a comparison within animal, within session and within day. Paired
across those sessions, V3 exceeded V1/V2 by +1.59 dB in beta (P=0.076), +1.16 dB in alpha (P=0.24)
and +0.56 dB in low gamma (P=0.24), and was marginally lower in high gamma (−0.17 dB, P=0.60). The
direction is the predicted one and the beta magnitude is large for a band-power contrast, but with
five sessions the contrast does not reach significance. We therefore report elevated alpha and beta
in V3 as a consistent, sizeable, single-animal observation, and not as an established area
difference. As above, this contrast is measured in the omitted-slot window and is expected to
strengthen when the post-omission delay is included. [[STAT: the same contrast in the union
window]]
...
Subject-level generalisation is provisional and, for area comparisons, structurally limited. No
cortical area in this corpus was recorded in all three animals, so no between-area coefficient is
separable from a between-animal difference. This is a property of the recording design rather than
of any particular analysis, and it cannot be repaired by modelling. Where an area contrast could be
tested with animal held constant, we report that contrast and no other.
