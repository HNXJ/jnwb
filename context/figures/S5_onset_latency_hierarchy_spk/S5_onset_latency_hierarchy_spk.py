...
Reuses jnwb.onset_fitting (causal_exp_smooth, fit_exponential_onset) UNCHANGED -- this is the
...
from jnwb.onset_fitting import causal_exp_smooth, fit_exponential_onset
from jnwb.unit_classification import omission_events, precompute_condition_onsets
...
def build_stats(run_out: dict) -> dict:
    ...
    return {
        ...
        "smoothing": "causal (forward-only) exponential kernel, jnwb.onset_fitting.causal_exp_smooth -- "
                     "per spec: acausal smoothing manufactures spurious early onsets",
        "onset_fit": "jnwb.onset_fitting.fit_exponential_onset -- causality-bounded (t0 cannot leave "
...
def _synthetic_area_spike_trains(t0_ms: float, tau_ms: float, amp_hz: float, base_hz: float,
    ...
    from jnwb.onset_fitting import onset_model
    rng = np.random.default_rng(seed)
