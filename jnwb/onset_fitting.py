...
def causal_exp_smooth(rate: np.ndarray, bin_ms: float, tau_ms: float = DEFAULT_TAU_MS) -> np.ndarray:
    ...
    threshold crossing -- it fits t0 as the takeoff parameter of ``onset_model`` -- so
    subtracting a filter delay from its t0 corrupts an estimate that was already right.
    Sweeping tau_ms tells the two readouts apart: on a 1 ms unit step the half-amplitude
    crossing tracks the filter at about tau_ms*ln(2), while the fitted t0 holds a tau-invariant
    -0.9 ms that follows bin_ms instead, measured at -0.118, -0.365, -0.860, -1.854 and
    -4.805 ms for bin_ms of 0.25, 0.5, 1, 2 and 5.
