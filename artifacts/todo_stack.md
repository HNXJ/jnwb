...
### 06-141 A result with no estimate is not reported as a number

Release: required-0.2.6.
Role: jnwb-developer. Skill: per skill. Blocked by: none.
Writes: `jnwb/connectivity.py`, `jnwb/statistics.py`, the module owning `phase_locking_index`, `skills/*/SKILL.md`, `tests/test_skill_decline_behaviour.py`, `CHANGELOG.md`.
P-250, P-251, P-252, P-253. Each function returns an ordinary-looking value (0.0, p 1.0) where no
estimate exists. Report it as the package already does for a non-identifiable result: NaN with the
existing flag, as `exploratory_correlate` does for a constant input.
Discriminator: each case's live value is pinned by the decline tests; the repair flips the pin.
Accept: the four rows close on verification; skill rows and CHANGELOG state the new values.
Stop: a function's documented contract promises the old value to callers.
...
Release: required-0.2.6.
Role: critic. Skill: none. Blocked by: 06-17, 06-53, 06-129, 06-136, 06-141. Writes: none.
