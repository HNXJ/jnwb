...
- 0.2.0-05c: Reconcile GPU fallback observability and population-trajectory API divergence. Standardize package GPU fallback warning/device provenance where the public return type permits it. Independently determine the intended semantics of analyzers.population_trajectory versus trajectory.compute_population_trajectory before changing either: covariance PCA and correlation PCA are distinct estimands; do not silently select one. Reproduce their divergence, inspect call sites/docs/history, then either consolidate only if semantic identity is established or expose the distinction explicitly with discriminating regression tests.
