...
- IA-27: `jnwb.vis.plot_granger_spectra` and `plot_rsm_heatmap` are at 0% coverage; the other exports this row named are pinned, and `nam.train_nam`'s training path is a declared exclusion because torch is in no CI leg. Check: one value-pinning test for each plot.
