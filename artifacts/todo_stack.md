...
- **Not covered, and why** Composition's *aggregation-order* and *identifier-survival* claims were not swept: no instrument was built to compare a multi-step skill workflow against an executed equivalent, and those remain with `tests/test_skill_routing_behaviour.py` (05-67, six rows executed), `tests/test_docs_smoke.py` and `tests/test_docs_nwb_workflow.py`. *Shape* was covered only where it reduces to axis order. Comparing declared return shapes against returned ones was attempted, with input dimensions chosen mutually distinct (5 channels, 7 trials, 512 samples, 11 frequencies) so no two axes could be confused, and the attempt is recorded as inconclusive rather than clean: only 3 of 29 candidate functions could be called from introspection alone, and both apparent mismatches were the probe's own -- it fell back to matching shape claims anywhere in the docstring when there was no `Returns:` block, so it read `bipolar_reference`'s input claim, and it handed `compute_psd` a channel-major array. A real sweep of this dimension needs per-function fixtures, not introspection. A capability-by-capability matrix over all nine dimensions was not built; the sweep is by dimension across the API instead, which finds a contradiction wherever it is but does not certify that every capability was examined on every dimension.
