...
### 06-145 `bin_spikes` refuses a window that is not whole bins

Release: required-0.2.6.
Role: actor. Skill: jnwb-spiking. Blocked by: none. Writes: `jnwb/connectivity.py`, `jnwb/viz.py`, `tests/test_connectivity*.py`, `CHANGELOG.md`, `skills/jnwb-connectivity/SKILL.md`, `skills/jnwb-spiking/SKILL.md`.
`bin_spikes` makes `round(span / bin)` bins; when that rounds up, the last edge passes the window end, spikes past the end are dropped, and `output='rate'` divides the partial bin by the full width, so a steady train reads low in the last bin. It is the binner every spike estimator uses. The 2026-09-23 PSTH ruling refuses such a window; the same refusal applies here through the shared whole-bin helper.
...
- P-293: `inspect(nwb)` on a never-written `NWBFile` leaves behavior and `FilteredEphys` containers unwrapped (acquisition `series`/`data_path` None, processing behavior entries dropped) where the file form reports them, against the `_continuous_entry_pynwb` docstring; no test exercises that path's layout call. Deferred: discovery output incomplete, not wrong-valued, only for unwritten files, older than this cycle.
...
- P-296: Dangling internal references in `jnwb/`: `nwb_tfr_storage_spec.md` (`compression.py:14`, `__init__.py:49,52`, untracked) and `artifacts/benchmarks/...` (`laminar.py:78`, `spectral.py:486,966`, pruned from the sdist); a person's name at `artifact_detection.py:7` and "Ruled 2026-09-20" at `nwb_io.py:63`. Deferred: no behavioural or scientific effect.
