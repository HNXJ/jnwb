...
- 0.2.3-REV-05: Fix asymmetric NaN omission in `aggregate_to_db(how="ratio_of_means", nan_policy="omit")`: apply joint finite masking across signal and baseline arrays before summation so numerator and denominator aggregate identical sample sets (EXT-REV-005).
