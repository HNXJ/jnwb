...
- 0.2.0-05a: Resolve addressing & metadata silent-science defects: eliminate fabricated 'probeA' default in enrich_units_dataframe (assign None), remove redundant stable_plus alias, support categorical/string quality in is_stable, decouple classify_unit_quality critical flags from hardcoded 1.0 thresholds, and document 0<=z<=20000 um / 1000 um model bounds in classify_layer_from_depth.
- 0.2.0-05b: Resolve jRSA input safety & redundant estimator fallback: raise ValueError on vector length mismatch in _pearson, _spearman, _kendall, and _cosine (eliminate silent min-length truncation); replace hand-rolled statsmodels ImportError fallback with jnwb.StatisticalAnalysis.fdr_correct.
- 0.2.0-05c: Reconcile GPU fallbacks & population trajectory estimators: enforce warn_device_fallback and explicit device_used return in spectral.py / trajectory.py; unify divergent centering vs standardization PCA implementations between analyzers.py and trajectory.py.
