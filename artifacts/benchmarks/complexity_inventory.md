...
| INV-05 | `phase_slope_index` | `jnwb.connectivity` | $\mathcal{O}(T \log N_{\text{perseg}} + K_{\text{seg}} \cdot F)$ | $\mathcal{O}(F \cdot K_{\text{seg}})$ | Segment FFTs and the coherency slope; the default leave-one-segment-out jackknife forms each left-out mean from prefix and suffix sums over the segments, so its $K_{\text{seg}}$ replicates cost $\mathcal{O}(K_{\text{seg}} \cdot F)$ together (`jackknife=False` drops it) | Nolte et al. 2008 |
...
| INV-14 | `stream_npz_array` | `jnwb.io` | $\mathcal{O}(E)$ | $\mathcal{O}(\text{slice volume})$ | A stored (uncompressed) member is seeked past what the slice skips, so its time follows the slice volume; a compressed member cannot be seeked and is read forward in fixed-size chunks up to its last selected element, so position bounds the time, and $\mathcal{O}(E)$ covers both | ZIP and `.npy` format specifications |
---
