...
| P-24 | `mkdocs.yml:25-26` declares exactly one palette scheme, `slate`. There is no light scheme and no toggle. Item 06-52 accepts on "renders legibly in both the light and the dark MkDocs theme" and discriminates by "switch the theme" -- neither is executable against this tree, because there is no light theme to switch to. An item's acceptance and the tree contradict each other. | This session, measuring for 06-48 | Open — adding a light scheme is new user-visible behaviour and is a scope decision, not a repair |
...
| P-26 | 7 documentation pages embed raster PNG figures from `docs/assets/figures/`. A documentation-form rule requiring inline SVG would forbid something 7 existing pages do, which is 06-48's own stated stop condition for a wrong rule. Either the rasters are debt that 06-52 clears, or the rule is wrong. The two readings are not distinguishable from the stack as written. | This session, measuring for 06-48 | Open — depends on P-24 |
