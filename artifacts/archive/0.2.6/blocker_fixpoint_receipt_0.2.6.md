...
| Field | Value |
|---|---|
| commit | `f0d905bf2feb0277d1c91130e7f2888100a81db7` |
...
| Closure over `6c651511..f0d905bf`: the release gate's installed-wheel smoke step expected a `wpli` key set that predates `device_used`, so it failed on a correct wheel; the fix, the sealed todo stack and this receipt | `f0d905bf` | PASS | none |

Every finding that is not a regression from these repairs is a `deferred-0.2.7` entry in `artifacts/todo_stack.md` (P-332 to P-353). The too-small p-values of other permutation nulls (P-345) lead the 0.2.7 work.

## Receipts at `6c651511` and `f0d905bf`
...
| At `f0d905bf` | Full suite 4830 passed, 0 failed; harness gates 19 PASS; closure critic killed 3 of 3 mutants of the smoke assertion and found 0 new release-blocking problems |
