...
- **Permutation exchangeability is the caller's responsibility.** The `permutations=1000` null
  built inside `jrsa()` has no grouping/exchangeability awareness -- it does not know about
  session, cycle, or subject structure in `x1`/`x2`. If the inputs carry that structure (e.g.
  concatenated across sessions or repeated temporal cycles), a naive permutation null can be
  invalid for the same reason `jnwb.omission_identity.decode_identity_cycle_deconfound`'s null
  was (audit-flagged 2026-08-10, `artifacts/.lab/agent-harness-audit-20260810.json`): comparing
  a statistic that respects real grouping against a null that ignores it. Pre-shuffle the
  grouped structure yourself with `jnwb.permutation.permute_labels(..., scheme="within_group")`
  and pass `permutations=0` (bootstrap your own null loop) if `x1`/`x2` are not exchangeable at
  the sample level.
