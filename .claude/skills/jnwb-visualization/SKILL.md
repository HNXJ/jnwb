...
`libcairo-2.dll`, confirmed again 2026-07-29). `reportlab.graphics.renderPM` (PNG output) also
fails (`ModuleNotFoundError: No module named '_rl_renderPM'`). `mcp__Claude_Browser__navigate`
to a `file://` SVG path and a local `http.server` + browser preview have both failed in this
environment — don't retry those.

**Simplest working path, confirmed 2026-07-29 across an entire figure-pipeline session**: every
panel-writing helper in `context/figures/figstyle.py` (`save()`) and every figure script in
`context/figures/` writes a `.png` companion next to every `.svg` it produces (matplotlib
`fig.savefig()` called twice, once per extension) — just `Read` the `.png` directly. This
covers every panel actually generated by the current figure pipeline and needs no conversion
step at all.

The svglib→PDF→Read path below is only needed for an SVG that has **no PNG companion** —
typically a hand-assembled or Illustrator-sourced SVG (figure 1's panels, an assembled
`figNN.svg` built by `svgassemble.py` from panel SVGs rather than drawn directly by matplotlib):
...
"Exported without visual inspection" does not satisfy the project's figure-verification
checklist below.
...
# dB re baseline: average power first, divide by baseline, THEN 10*log10 ONCE -- never average
# in dB space, never log before dividing by baseline. See jnwb-tfr/SKILL.md "Take the logarithm
# last" for the full rationale and the exact bug (jnwb/session.py's plot_tfr) this prevents.
power_db = 10.0 * np.log10(np.maximum(mean_power, 1e-12) / np.maximum(baseline, 1e-12))
# then ax.pcolormesh(times, freqs, power_db, ...)
```
...
`session.trial_averaged_plot` and `session.plot_tfr` (Task 9) now compute this identically
(verified 2026-08-10 to agree exactly, max abs diff 0.0) -- both average power first, divide by
a fixed 200ms pre-stimulus baseline, then take `10*log10` once. If you write a THIRD TFR-dB
plotting path, it must match this same order or it will silently disagree with these two.
```python
...
## Publication figures now live in a separate, canonical pipeline — do not duplicate colors/areas here

`context/figures/` is the current source of truth for every manuscript figure (figs 1-7 plus
supplements). It has its own shared modules — `figstyle.py` (timing, band/area/class colors,
epoch shading, Cambria, exact binomial interval), `svgassemble.py` (panel layout + namespacing),
`figstats.py` (the statistics harness: parametric/non-parametric choice, Holm + BH corrections,
effect sizes) — and a `context/figures/README.md` documenting the one-folder-per-figure
convention (`figNN_<analysis>.py`, `svg/`, assembled `figNN.svg`, `build_supplements.py`).

**Before writing or editing any figure that will ship in the manuscript, read
`context/figures/README.md` and import from `figstyle.py`/`figstats.py` rather than
hardcoding a color, area, or timing constant here or anywhere else.** A previous version of
this section hardcoded "S+ Candidate (MT)" / "S- Candidate (V2)" as canonical exemplar
choices and a different epoch-shading palette; both were superseded by 2026-07-29 (S+/S-/O+/
O++ are now V1/V3a/d/V4/FEF respectively, fixed by figure spec, and shading uses
`figstyle.SLOT_COLORS`). Hardcoding these facts in two places is exactly how they drifted
apart the first time — the fix is to point at the code, not to re-fix the literal again.

The mplstyle example below remains useful for ad-hoc/exploratory jnwb plotting (outside the
manuscript figure pipeline), where `context/figures/figstyle.use_house_style()` does not apply.

### Matplotlib Visual Style Automation (`.mplstyle`) — exploratory plotting only
To automatically enforce styling parameters across ad-hoc Matplotlib plotting without manually
hardcoding parameters on every axis, define a local stylesheet asset:
...
## Session reports (`jnwb.report`, `jnwb.markdown_report`)

Two report generators, different outputs — do not confuse them.

```python
from jnwb.report import generate_report, generate_notebook_json, apply_madelane_style, compute_psd, fdr_correct
out_dir = generate_report(nwb_path_or_id, output_parent_dir="artifacts/reports")
```

`generate_report` accepts a full path *or* a bare session id — if the path does not exist it
searches `oa.paths.nwb_dir()` for a match. `apply_madelane_style(ax, ...)` is the house axis
style; use it rather than restyling axes by hand.

```python
from jnwb.markdown_report import generate_markdown_report
md = generate_markdown_report(session_name, audit_csv_path, highlight_units,
                              source_svg_dir, output_root)
```

`generate_markdown_report` produces multi-page markdown with embedded SVG — it consumes figures
already written to `source_svg_dir`, it does not render them. Generate figures first.

Note `jnwb.report.fdr_correct` is a module-level helper, distinct from
`StatisticalAnalysis.fdr_correct` in `jnwb-statistics`. Pick one per analysis and say which.
