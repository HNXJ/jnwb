...
computation isn't possible, and must call the real jnwb.decoding /
...
from jnwb.factories import result_from_decoding_analysis, result_from_tfr_analysis, dataset_from_session
...
class TestDecodingFactoryNoFabrication:
    ...
    def test_real_decode_stimulus_identity_is_called_when_epochs_b_given(self):
        ...
        import jnwb.factories as factories_mod
        original = factories_mod.decode_stimulus_identity if hasattr(factories_mod, "decode_stimulus_identity") else None

        with pytest.MonkeyPatch.context() as mp:
            mp.setattr("jnwb.decoding.decode_stimulus_identity", MagicMock(return_value=fake_dec_result))
